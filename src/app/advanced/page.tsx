...
const AdvancedContent: React.FC = () => {
  ...
  const handleGenerateCertificate = async () => {
    if (progress.advanced.quizScore) {
      const certificateData: CertificateData = {
        score: Math.round((progress.advanced.quizScore / 100) * 20), // Convert percentage to score out of 20
        totalQuestions: 20,
        percentage: progress.advanced.quizScore,
        completionDate: new Date().toLocaleDateString(),
        difficulty: 'advanced'
      };
      
      const dataUrl = await generateCertificate(certificateData);
      downloadCertificate(dataUrl, 'zama-advanced-certificate.png');
    }
  };
